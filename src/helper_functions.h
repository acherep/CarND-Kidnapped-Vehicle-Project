...
/*
 * Computes the 2D-Gaussian PDF with mean (mu_x, mu_y)
 * and standard deviation (std_x, 0; 0, std_y)
 * at the observation point (x_obs, y_obs) .
 * @param (x_obs, y_obs) x and y coordinates of the observation
 * @param (mu_x, mu_y) x and y mean of the 2D-Gaussian distribution
 * @param (std_x, std_y) x and y standard deviations of the 2D-Gaussian
 * distribution
 * @output 2D-Gaussian PDF value
 */
inline double gaussian2D(double x_obs, double y_obs, double mu_x, double mu_y,
                         double std_x, double std_y) {
  // calculate normalization term
  double gauss_norm = 1 / (2 * M_PI * std_x * std_y);

  // calculate exponent
  double exponent = (x_obs - mu_x) * (x_obs - mu_x) / (2 * std_x * std_x) +
                    (y_obs - mu_y) * (y_obs - mu_y) / (2 * std_y * std_y);
